...
The production image refuses to start when either required share is missing. Use `npm run deploy:container -- --image <registry/image:tag>` for every Container Apps image rollout; it reapplies the checked-in mounts and one-replica boundary. The command sends all traffic to that exact healthy revision. After deployment, run `npm run verify:deployment` with Azure access to check the active revision, mounts, replica count, and six-request rate limit.
